...
public class ChatController {
  ...
  public ChatController(
      ChatService chatService, @Value("${spring.ai.openai.api-key:#{null}}") String openAiApiKey) {
  ...
  public Flux<String> chat(
      @PathVariable(required = false) String expertType,
    ...
    String effectiveExpertType =
        StringUtils.hasText(expertType) ? expertType.toLowerCase() : DEFAULT_EXPERT;
    String effectiveProvider = determineProvider(provider);

    log.info(
        "Processing chat request: expertType={}, message={}, provider={}",
        effectiveExpertType,
        message,
        effectiveProvider);
    return switch (effectiveExpertType) {
      ...
      default -> {
        ...
        yield Flux.error(
            new IllegalArgumentException("Invalid expert type: " + effectiveExpertType));
      }
    };
  }
  ...
  private String determineProvider(String providedProvider) {
    ...
    String selectedProvider =
        StringUtils.hasText(openAiApiKey) ? OPENAI_PROVIDER : DEFAULT_PROVIDER;
